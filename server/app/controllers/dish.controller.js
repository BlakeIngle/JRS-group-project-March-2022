...
exports.getAllDishes = (req, res) => {
    ...
    db.query(query, (err, results) => {
        if (err) {
            res.status(500)
                .send({ error: err, message: "Error retrieving dishes." })
        } else {
            if (results.length == 0) {
                res.status(404)
                    .send({ message: "No dishes found." });
            } else {
...
exports.getDishById = (req, res) => {
    ...
    db.query(query, placeholders, (err, results) => {
        if (err) {
            res.status(500)
                .send({ error: err, message: "Error retrieving dish." })
        } else {
            if (results.length == 0) {
                res.status(404)
                    .send({ message: "No dish found." });
            } else {
...
exports.getDishByLocation = (req, res) => {
    ...
    const query = `SELECT * FROM dishes.dish WHERE location = ?;`;
...
exports.addNewDish = (req, res) => {
    ...
    db.query(query, placeholders, (err, results) => {
        if (err) {
            res.status(500)
                .send({ error: err, message: "Error posting dish." })
        } else {
            res.send({
...
exports.updateDish = (req, res) => {
    ...
    db.query(query, placeholders, (err, results) => {
        if (err) {
            res.status(500)
                .send({ error: err, message: "Error updating dish." })
        } else {
            res.send({
...
exports.deleteDishById = (req, res) => {
    ...
    db.query(query, placeholders, (err, results) => {
        if (err) {
            res.status(500)
                .send({ error: err, message: "Error deleting dish." })
        } else {
            res.send({
